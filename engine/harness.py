...
def build_context(target: Path, bootstrap_root: Path, intent: str = "") -> dict:
    ...
    research = build_agenda(facts, architecture, intent, memory)
...
def render_project_md(context: dict) -> str:
    ...
    manual = []
    for key, item in context.get("memory", {}).get("facts", {}).items():
        if item.get("source", {}).get("type") == "manual":
            manual.append("- `{}`: `{}` (verified: {}).".format(key, item.get("value"), item.get("source", {}).get("reference", "manual")))
    manual_text = "\n".join(manual) or "- No manually verified project facts recorded yet."
...
## Manually verified project facts
